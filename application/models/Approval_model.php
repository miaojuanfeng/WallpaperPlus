...
class Approval_model extends CI_Model {
	...
	{
		$data['setting_modify'] = date('Y-m-d H:i:s');
		$data = get_array_prefix('setting_', $data);
		$this->db->where('setting_id', $data['setting_id']);
		$thisResult = $this->db->update('setting', $data); 
			...
			foreach($data['where'] as $key => $value){
				switch($key){
					...
					case 'setting_name':
						...
						break;
					case 'setting_id_like':
					case 'setting_name_like':
						$thisField = str_replace('_like', '', $key);
						$this->db->like($thisField, urldecode($value));
						break;
					case 'setting_id_noteq':
					case 'setting_name_noteq':
	...
	function structure()
	{
		$query = $this->db->query("show full columns from setting");
		return $query->result();
	}
